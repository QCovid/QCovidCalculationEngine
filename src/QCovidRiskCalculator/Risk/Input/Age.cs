...
namespace QCovid.RiskCalculator.Risk.Input
{
    ...
    public class Age
    {
        ...
        /// <summary>
        /// Get a users age from their date of birth and a specified current date.
        /// </summary>
        /// <remarks>
        /// Assumes users follow the western idea of age, e.g. not East Asian age reckoning.
        /// Assumes users born on February 29 consider their birthday to be March 1 in non-leap years.
        /// </remarks>
        /// <param name="dateOfBirth"></param>
        /// <param name="localNow"></param>
        public static Age CreateFromDateOfBirth(Date dateOfBirth, Date localNow)
        {
            int years = localNow.Value.Year - dateOfBirth.Value.Year;

            // AddYears understands leap years
            if (dateOfBirth.Value > localNow.Value.AddYears(-years))
            {
                years--;
            }
