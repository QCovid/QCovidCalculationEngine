...
{
    internal class ChemotherapyGroupMapping : EnumMapping<CancerTreatmentsAndImmunoSuppressants, ChemotherapyGroup>
    {
        public ChemotherapyGroupMapping(Expression<Func<CancerTreatmentsAndImmunoSuppressants, ChemotherapyGroup>> childExpression)
            : base(GetCodeGroupIdValuePairs(), ri => ri.ClinicalInformation.CancerTreatmentsAndImmunoSuppressants, childExpression)
        {
        }

        public override string ToStringProcessingRules() => "Most severe group present in the last 12 months";
        ...
        {
            DateTime earliestDate = processingReferenceDate.Value.AddMonths(-12);
            var instancesWithinPeriod = recognisedInstances
                .Where(cgi => cgi.Date.Value >= earliestDate)
                .ToArray();

            if (instancesWithinPeriod.Any())
            {
                ChemotherapyGroup mostSevere = instancesWithinPeriod
                    .Max(GetValueFromCodeGroupInstance);

                SetValue(riskInput, mostSevere);
            }
        }

        private static IReadOnlyList<(int[] ids, ChemotherapyGroup value)> GetCodeGroupIdValuePairs()
        {
            return new[]
            {
                (new[]{MedicalCodeDbConstants.ChemotherapyGroupAId}, ChemotherapyGroup.GroupA),
                (new[]{MedicalCodeDbConstants.ChemotherapyGroupBId}, ChemotherapyGroup.GroupB),
                (new[]{MedicalCodeDbConstants.ChemotherapyGroupCId}, ChemotherapyGroup.GroupC),
            };
        }
    }
}
