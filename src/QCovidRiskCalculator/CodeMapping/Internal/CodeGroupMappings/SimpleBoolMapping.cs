...
{
    // <summary>
    // The trivial case of: if the code appears then the input is set to true
    // </summary>
    // <typeparam name="TParent"></typeparam>
    internal class SimpleBoolMapping<TParent> : BoolMapping<TParent>
        where TParent : class
    {
        public SimpleBoolMapping(IReadOnlyList<int> codeGroupIds, Expression<Func<ClinicalInformation, TParent>> parentExpression, Expression<Func<TParent, bool>> childExpression)
            : base(codeGroupIds, parentExpression, childExpression)
        {
        }

        public override string ToStringProcessingRules() => "True if present";
