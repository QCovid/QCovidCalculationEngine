...
{
    internal class MostRecentInstanceEnumMapping<TParent, TEnum> : EnumMapping<TParent, TEnum>
        where TEnum : class
        where TParent : class
    {
        public MostRecentInstanceEnumMapping(IReadOnlyList<(int[] ids, TEnum value)> codeGroupIdValuePairs, Expression<Func<RiskInput, TParent>> parentExpression, Expression<Func<TParent, TEnum>> childExpression) 
            : base(codeGroupIdValuePairs, parentExpression, childExpression)
        {
        }

        public override string ToStringProcessingRules() => "Most recent instance used";
        ...
        {
            CodeGroupInstance mostRecent = recognisedInstances.OrderByDescending(cgi => cgi.Date).First();

            SetValue(riskInput, mostRecent);
        }
    }
}
